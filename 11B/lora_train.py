...
def train():
    ...
    # Optionally load a previous checkpoint (for base weights), if it exists
    if config.load_from_checkpoint and os.path.exists(config.checkpoint_dir):
        logger.info(f"Loading base model from checkpoint: {config.checkpoint_dir}")
        try:
            model = AutoModelForCausalLM.from_pretrained(
            ...
            # Make sure weights are tied if needed
            if hasattr(model, 'tie_weights'):
                model.tie_weights()
            logger.info("Successfully loaded checkpoint for base model")
        except Exception as e:
            logger.error(f"Error loading base checkpoint: {e}")
            raise
    
    # ---------------------------------
    # 1) Create LoRA config and wrap the model with PEFT
    # ---------------------------------
    lora_config = LoraConfig(
        ...
        lora_alpha=16,
        lora_dropout=0.05,
        bias="none"                   # or "lora_only" etc.
    )
    
    model = get_peft_model(model, lora_config)
    
    # (Optional) Print trainable params vs. frozen
    ...
    # We only want to pass the trainable parameters (LoRA) to DeepSpeed
    ...
    # Create dataloader (already given to DS, but we may want it for more control)
    ...
    for epoch in range(config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if model_engine.global_rank == 0 and (step % config.logging_steps == 0):
    ...
    if model_engine.global_rank == 0:
        logger.info("Training completed. Saving LoRA adapter...")
        
        # The LoRA weights live in model_engine.module after training
        final_output_dir = os.path.join(config.output_dir, "lora")
        os.makedirs(final_output_dir, exist_ok=True)
        
        # Save LoRA adapter
        model_engine.module.save_pretrained(final_output_dir)
        
        # Save the tokenizer as well
        tokenizer.save_pretrained(final_output_dir)

        logger.info(f"LoRA adapter and tokenizer saved in {final_output_dir}")
